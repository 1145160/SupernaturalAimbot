...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextMoveToPoint(ctx, cx-15, cy); CGContextAddLineToPoint(ctx, cx-5, cy);
    CGContextMoveToPoint(ctx, cx+5, cy); CGContextAddLineToPoint(ctx, cx+15, cy);
    CGContextMoveToPoint(ctx, cx, cy-15); CGContextAddLineToPoint(ctx, cx, cy-5);
    CGContextMoveToPoint(ctx, cx, cy+5); CGContextAddLineToPoint(ctx, cx, cy+15);
    CGContextStrokePath(ctx);
    
    CGContextAddEllipseInRect(ctx, CGRectMake(cx-100, cy-100, 200, 200));
    CGContextSetStrokeColorWithColor(ctx, [UIColor colorWithRed:1 green:0.3 blue:0 alpha:0.5].CGColor);
    CGContextSetLineWidth(ctx, 1.5);
    CGContextStrokePath(ctx);
    
    if (killText && [[NSDate date] timeIntervalSince1970] < killTextEnd) {
        NSString *text = [NSString stringWithFormat:@"消灭 %@", lastKillName];
        UIFont *font = [UIFont boldSystemFontOfSize:24];
        NSDictionary *attrs = @{NSFontAttributeName: font, NSForegroundColorAttributeName: [UIColor redColor]};
        CGSize size = [text sizeWithAttributes:attrs];
        [text drawAtPoint:CGPointMake(cx - size.width/2, cy - 100) withAttributes:attrs];
    }
    
    [[NSString stringWithFormat:@"存活 %d/16", totalAlive] drawAtPoint:CGPointMake(15, 50) withAttributes:@{NSFontAttributeName: [UIFont boldSystemFontOfSize:12], NSForegroundColorAttributeName: [UIColor whiteColor]}];
    [[NSString stringWithFormat:@"可视 %d", visibleEnemies] drawAtPoint:CGPointMake(15, 70) withAttributes:@{NSFontAttributeName: [UIFont systemFontOfSize:11], NSForegroundColorAttributeName: [UIColor yellowColor]}];
    [[NSString stringWithFormat:@"锁定 %d", currentTarget ? 1 : 0] drawAtPoint:CGPointMake(15, 90) withAttributes:@{NSFontAttributeName: [UIFont systemFontOfSize:11], NSForegroundColorAttributeName: [UIColor cyanColor]}];
    
    NSString *modeText = [NSString stringWithFormat:@"模式 %@", currentConfig.name];
    [modeText drawAtPoint:CGPointMake(SCREEN_WIDTH - 85, 50) withAttributes:@{NSFontAttributeName: [UIFont boldSystemFontOfSize:11], NSForegroundColorAttributeName: currentConfig.color}];
    ...
    if (currentTarget && currentTargetScreen.x > 0 && currentTargetScreen.y > 0) {
        ...
        CGContextSetStrokeColorWithColor(ctx, [UIColor colorWithRed:1 green:0.3 blue:0 alpha:0.6].CGColor);
        CGContextSetLineWidth(ctx, 2);
        CGContextMoveToPoint(ctx, cx, cy);
        CGContextAddLineToPoint(ctx, ex, ey-25);
        CGContextStrokePath(ctx);
        
        CGRect bg = CGRectMake(ex-40, ey-55, 80, 8);
        ...
        CGContextFillRect(ctx, CGRectMake(ex-40, ey-55, 80 * healthPercent, 8));
        
        NSString *name = getName(currentTarget);
        UIFont *font = [UIFont boldSystemFontOfSize:11];
        [name drawAtPoint:CGPointMake(ex - [name sizeWithAttributes:@{NSFontAttributeName: font}].width/2, ey-68) withAttributes:@{NSFontAttributeName: font, NSForegroundColorAttributeName: [UIColor whiteColor]}];
        
        NSString *dist = [NSString stringWithFormat:@"%.0fm", currentTargetDistance];
        [dist drawAtPoint:CGPointMake(ex - [dist sizeWithAttributes:@{NSFontAttributeName: font}].width/2, ey-45) withAttributes:@{NSFontAttributeName: [UIFont systemFontOfSize:10], NSForegroundColorAttributeName: [UIColor yellowColor]}];
        
        NSString *icon = (currentTargetDistance <= 80) ? @"🔫" : @"🎯";
        [icon drawAtPoint:CGPointMake(ex-25, ey-70) withAttributes:@{NSFontAttributeName: [UIFont systemFontOfSize:14]}];
    }
}
...
static void setupUI() {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        DrawView *view = [[DrawView alloc] initWithFrame:overlayWindow.bounds];
